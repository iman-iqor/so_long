...
{
    t_game* game = imane->game;
    game->player_img = mlx_xpm_file_to_image(game->mlx, "pics/player.xpm", &game->img_width, &game->img_height);
    ...
    game->background_img = mlx_xpm_file_to_image(game->mlx, "pics/background1.xpm", &game->img_width, &game->img_height);

    if (!game->player_img || !game->coin_img|| !game->wall_img || !game->exit_img || !game->background_img)
    {
